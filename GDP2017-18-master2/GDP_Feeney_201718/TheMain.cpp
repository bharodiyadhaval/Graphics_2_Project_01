...
cFBO g_FBO_Myscreen1_G_Buffer;
cFBO g_FBO_Myscreen1_Deferred;
...
cGameObject* g_RoomMaskForStencil2 = NULL;
cGameObject* shiny = NULL;
...
//	GLint uniLoc_diffuseColour = glGetUniformLocation( currentProgID, "diffuseColour" );
	...
	::g_pLightManager->vecLights[1].attenuation.z = 0.000456922280f;		//0.172113f;
	...
	//vecLigh
	//::g_pLightManager->vecLights[1]. = glm::vec3(0.0f, 50.0f, 0.0f);

	/*::g_pLightManager->vecLights[2].setLightParamType(cLight::POINT);
	::g_pLightManager->vecLights[2].position = glm::vec3(500.0f, 500.0f, 1000.0f);
	::g_pLightManager->vecLights[2].attenuation.y = 1.0f;
	::g_pLightManager->vecLights[2].attenuation.x = 1.0f;
	::g_pLightManager->vecLights[2].attenuation.z = 1.0f;
*/
...
//	::g_pTheCamera->eye = glm::vec3(0.0f, 5.0f, -10.0f);
	...
	::g_pTheCamera->FlyCamLA->setEyePosition(glm::vec3(0.0f, 0.0f, 2000.0f));
	::g_pTheCamera->FlyCamLA->setTargetInWorld(glm::vec3(0.0f, -60.0f, 0.0f));
...
//	::g_pTheCamera->FollowCam->SetOrUpdateTarget(glm::vec3(1.0f));
	...
	if (!::g_FBO_Myscreen1_G_Buffer.init(1920, 1080, error))
	{
		std::cout << "::g_FBO_Pass2_Deferred error: " << error << std::endl;
	}
	else
	{
		std::cout << "::g_FBO_Pass2_Deferred is good." << std::endl;
		std::cout << "\t::g_FBO_Myscreen1_G_Buffer ID = " << ::g_FBO_Myscreen1_G_Buffer.ID << std::endl;
		std::cout << "\tcolour texture ID = " << ::g_FBO_Myscreen1_G_Buffer.colourTexture_0_ID << std::endl;
		std::cout << "\tnormal texture ID = " << ::g_FBO_Myscreen1_G_Buffer.normalTexture_1_ID << std::endl;

		std::cout << "GL_MAX_COLOR_ATTACHMENTS = " << ::g_FBO_Myscreen1_G_Buffer.getMaxColourAttachments() << std::endl;
		std::cout << "GL_MAX_DRAW_BUFFERS = " << ::g_FBO_Myscreen1_G_Buffer.getMaxDrawBuffers() << std::endl;

	}//if ( ! ::g_FBO_Myscreen1_G_Buffer.init

	if (!::g_FBO_Myscreen1_Deferred.init(1920, 1080, error))
	{
		std::cout << "::g_FBO_Myscreen1_Deferred error: " << error << std::endl;
	}
	else
	{
		std::cout << "FBO is good." << std::endl;
		std::cout << "\t::g_FBO_Myscreen1_Deferred ID = " << ::g_FBO_Myscreen1_Deferred.ID << std::endl;
		std::cout << "\tcolour texture ID = " << ::g_FBO_Myscreen1_Deferred.colourTexture_0_ID << std::endl;
		std::cout << "\tnormal texture ID = " << ::g_FBO_Myscreen1_Deferred.normalTexture_1_ID << std::endl;

		std::cout << "GL_MAX_COLOR_ATTACHMENTS = " << ::g_FBO_Myscreen1_Deferred.getMaxColourAttachments() << std::endl;
		std::cout << "GL_MAX_DRAW_BUFFERS = " << ::g_FBO_Myscreen1_Deferred.getMaxDrawBuffers() << std::endl;

	}//if ( ! ::g_FBO_Myscreen1_Deferred.init
...
//			<< state.position.z << std::endl;
		...
		::shiny->adjQOrientation(glm::quat(glm::vec3(0,deltaTime*2, 0)));
//    ___                _             _           ___       _            __   __           
...
//	glDepthMask(GL_FALSE);
		...
		vecOnlyTheRoomMask.push_back(::g_RoomMaskForStencil2);
		RenderScene(vecOnlyTheRoomMask, ::g_pGLFWWindow, deltaTime );


		//Sciss
		RenderScene(vecOnlyTheRoomMask, ::g_pGLFWWindow, deltaTime);
		/*glScissor(100, 400, 500, 300);
...
//	glDepthMask(GL_TRUE);
		...
		vecOnlyTheRoom.push_back(::shiny);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		{
			...
			glBindFramebuffer(GL_FRAMEBUFFER, g_FBO_Myscreen1_G_Buffer.ID);
			// Clear colour AND depth buffer
			g_FBO_Myscreen1_G_Buffer.clearBuffers();
			...
			//if (::camflag == 1)
			//{
				::my_camera->FlyCamLA->setEyePosition(glm::vec3(500.0f, 0, 500.0));

			//}
			::my_camera->FlyCamLA->setTargetInWorld(glm::vec3(500.0f, 25.0f, 0.0f));
			...
			//std::vector<cGameObject*> tempGO;
			//tempGO.push_back(::g_vecGameObjects[3]);
			//RenderScene(tempGO, ::g_pGLFWWindow, deltaTime);
			RenderScene(::g_vecGameObjects, ::g_pGLFWWindow, deltaTime);
			...
			//DEFFERED STUFF   
			//g_FBO_Myscreen1_Deferred
			glBindFramebuffer(GL_FRAMEBUFFER, g_FBO_Myscreen1_Deferred.ID);
			g_FBO_Myscreen1_Deferred.clearBuffers();
			...
			GLint texFBOColour2DTextureUnitID = 13;
			GLint texFBOColour2DLocID = glGetUniformLocation(sexyShaderID, "texFBOColour2D");
			GLint texFBONormal2DTextureUnitID = 14;
			GLint texFBONormal2DLocID = glGetUniformLocation(sexyShaderID, "texFBONormal2D");
			GLint texFBOWorldPosition2DTextureUnitID = 15;
			GLint texFBOWorldPosition2DLocID = glGetUniformLocation(sexyShaderID, "texFBOVertexWorldPos2D");

			// Pick a texture unit... 
			glActiveTexture(GL_TEXTURE0 + texFBOColour2DTextureUnitID);
			glBindTexture(GL_TEXTURE_2D, g_FBO_Myscreen1_G_Buffer.colourTexture_0_ID);
			glUniform1i(texFBOColour2DLocID, texFBOColour2DTextureUnitID);

			glActiveTexture(GL_TEXTURE0 + texFBONormal2DTextureUnitID);
			glBindTexture(GL_TEXTURE_2D, g_FBO_Myscreen1_G_Buffer.normalTexture_1_ID);
			glUniform1i(texFBONormal2DLocID, texFBONormal2DTextureUnitID);

			glActiveTexture(GL_TEXTURE0 + texFBOWorldPosition2DTextureUnitID);
			glBindTexture(GL_TEXTURE_2D, g_FBO_Myscreen1_G_Buffer.vertexWorldPos_2_ID);
			glUniform1i(texFBOWorldPosition2DLocID, texFBOWorldPosition2DTextureUnitID);
			...
			//::g_pTheCamera->FlyCamLA->setEyePosition(glm::vec3(0.0f, 0.0f, 650.0f));
			//::g_pTheCamera->FlyCamLA->setTargetInWorld(glm::vec3(0.0f, -60.0f, 0.0f));
...
// NOTE: In this example, we are only using this to render to an offscreen object
	...
	GLint myRenderedImage2D_LocID = glGetUniformLocation(sexyShaderID, "myRenderImage");
	...
	/////////////////////////////////////////////////////////////
	unsigned int pass2unit1 = 51;
	glActiveTexture(GL_TEXTURE0 + pass2unit1);
	glBindTexture(GL_TEXTURE_2D, ::g_FBO_Myscreen1_Deferred.colourTexture_0_ID);
	glUniform1i(myRenderedImage2D_LocID, pass2unit1);
...
//	vecCopySingleLonelyQuad.push_back( pTheShip );
	...
	cGameObject* pTheTV = findObjectByFriendlyName("myTV", ::g_vecGameObjects);
	std::vector<cGameObject* > myvectv;
	myvectv.push_back(pTheTV);
	RenderScene(myvectv, ::g_pGLFWWindow, deltaTime);
	cGameObject* pTheTV1 = findObjectByFriendlyName("myTV1", ::g_vecGameObjects);
	std::vector<cGameObject* > myvectv1;
	myvectv1.push_back(pTheTV1);
	RenderScene(myvectv1, ::g_pGLFWWindow, deltaTime);

		//RenderScene(::g_vecTransparentObjectsOnly, ::g_pGLFWWindow, deltaTime);

		std::stringstream ssTitle;
		ssTitle << " GRAPHICS 2 :: PROJECT 1 ";
//			<< g_cameraXYZ.x << ", " 
//			<< g_cameraXYZ.y << ", " 
//			<< g_cameraXYZ.z;
		// 
