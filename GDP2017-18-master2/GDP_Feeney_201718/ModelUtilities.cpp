...
//	if ( ! ::g_pSkinnedMesh01->LoadMeshFromFile( "assets/modelsFBX/RPG-Character_Unarmed-Fall(FBX2013).fbx" ) ) 
	...
	if (!Load3DModelHelper("HouseWithNormals.ply",
		"HouseWithNormals.ply",
		shaderID, pVAOManager, pModelAssetLoader, true, error))
	{
		std::cout << error << std::endl;
	}
	if (!Load3DModelHelper("tv_normals_uv.ply",
		"tv_normals_uv.ply",
		shaderID, pVAOManager, pModelAssetLoader, true, error))
	{
		std::cout << error << std::endl;
	}
	if (!Load3DModelHelper("backpanel_normals_uv.ply",
		"backpanel_normals_uv.ply",
		shaderID, pVAOManager, pModelAssetLoader, true, error))
	{
		std::cout << error << std::endl;
	}
	...
	{
		cMesh testMesh;
		testMesh.name = "HouseInside";
		if (!pModelAssetLoader->LoadPlyFileIntoMeshWith_Normals_and_UV("HouseWithNormals.ply", testMesh))
		{
			//std::cout << "Didn't load model" << std::endl;
			ssError << "Didn't load model >" << testMesh.name << "<" << std::endl;
			bAllGood = false;
	...
	}
	{
		cMesh testMesh;
		testMesh.name = "house";
		if (!pModelAssetLoader->LoadPlyFileIntoMeshWith_Normals_and_UV("Residence_unv..ply", testMesh))
		{
			//std::cout << "Didn't load model" << std::endl;
			ssError << "Didn't load model >" << testMesh.name << "<" << std::endl;
			bAllGood = false;
...

	{
		cMesh testMesh;
		testMesh.name = "tv";
		if (!pModelAssetLoader->LoadPlyFileIntoMeshWith_Normals_and_UV("retro.ply", testMesh))
		{
			//std::cout << "Didn't load model" << std::endl;
			ssError << "Didn't load model >" << testMesh.name << "<" << std::endl;
			bAllGood = false;
