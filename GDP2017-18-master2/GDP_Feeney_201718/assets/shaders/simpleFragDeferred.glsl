...
uniform sampler2D myRenderImage;
// ... and so on...
...
uniform bool ismyScreen;
uniform bool ismyScreen1;
...
{	
	...
	//		fragColourOut[0] * 1.5f;	// Room too bright
			FBOout_colour.rgb = 1 - materialDiffuse.rgb;			//gl_FragColor.rgb
...
// **************************************************************************
	...
	if ( ismyScreen )
		...
		vec2 textCoords = vec2( gl_FragCoord.x / screenWidth, gl_FragCoord.y / screenHeight );
			...
			FBOout_colour.rgb = texture(myRenderImage,fUV_X2.xy).rgb;
			float Y = (0.2126 * FBOout_colour.r) + 
		          (0.7152 * FBOout_colour.g) + 
				  (0.0722 * FBOout_colour.b);
		FBOout_colour.rgb = vec3(Y,Y,Y);
		...
		vec2 textCoords = vec2( gl_FragCoord.x / screenWidth, gl_FragCoord.y / screenHeight );
			...
			FBOout_colour.rgb = texture(myRenderImage,fUV_X2.xy).rgb;
			FBOout_colour.r = 1-FBOout_colour.r;
			FBOout_colour.g = 1-FBOout_colour.g;

			FBOout_colour.b = 1-FBOout_colour.b;
...
//		FBOout_colour.a = 1.0f;
		...
		FBOout_colour.rgb += ( outColour / float(count) );
			...
			//vec2 textCoords = vec2( 1.0f,100.0f);
			FBOout_colour.rgb = texture( fullRenderedImage2D, textCoords.xy ).rgb;
			FBOout_colour.r = 1.2 * FBOout_colour.r;
			FBOout_colour.g = 1.2* FBOout_colour.g;
			FBOout_colour.b = 1.2 *  FBOout_colour.b;
			FBOout_colour.a = 1.0f;
		}
