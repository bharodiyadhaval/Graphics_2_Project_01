...
//			GLint diffuseColour_loc = glGetUniformLocation( shaderID, "diffuseColour" );
	...
	GLint uniLoc_blur = glGetUniformLocation(curShaderProgID, "blurON");
	if (::blurON)
	{
		glUniform1f(uniLoc_blur, GL_TRUE );
...
//	else
	...
	GLint uniLoc_myScreen = glGetUniformLocation(curShaderProgID, "ismyScreen");
	if (pTheGO->myScreen)
	{
		glUniform1f(uniLoc_myScreen, GL_TRUE);
	}
	else
	{
		glUniform1f(uniLoc_myScreen, GL_FALSE);
	}

	GLint uniLoc_myScreen1 = glGetUniformLocation(curShaderProgID, "ismyScreen1");
	if (pTheGO->myScreen1)
	{
		glUniform1f(uniLoc_myScreen1, GL_TRUE);
	}
	else
	{
		glUniform1f(uniLoc_myScreen1, GL_FALSE);
	}
